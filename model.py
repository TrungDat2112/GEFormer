...
class GEFormerDTA(torch.nn.Module):
    def __init__(self, num_features_xd, num_features_xt,
                     latent_dim=64, dropout=0.2, n_output=1, device='cpu'):
        ...
        self.hidden_dim = 64
        self.inter_dim = 16
        self.multi_hop_max_dist = 10

        self.encoder_dropout = 0
        self.attention_dropout = 0
        self.input_dropout = nn.Dropout(0)

        self.d_node_encoder = nn.Embedding(128 * 9 + 1, self.hidden_dim, padding_idx=0)
        self.d_edge_encoder = nn.Embedding(128 * 3 + 1, self.num_heads, padding_idx=0)
        self.d_edge_dis_encoder = nn.Embedding(128 * self.num_heads * self.num_heads, 1)
        self.d_spatial_pos_encoder = nn.Embedding(128, self.num_heads, padding_idx=0)
        self.d_in_degree_encoder = nn.Embedding(128, self.hidden_dim, padding_idx=0)
        self.d_out_degree_encoder = nn.Embedding(128, self.hidden_dim, padding_idx=0)
        ...
        self.graph_token_virtual_distance = nn.Embedding(1, 2)
        ...
        self.fc_g1_d = torch.nn.Linear(num_features_xd*2, 128)
        self.fc_g2_d = torch.nn.Linear(128, num_features_xt)
        ...
        self.fc_g1_t = torch.nn.Linear(latent_dim * 2, 128)
        self.fc_g2_t = torch.nn.Linear(128, latent_dim * 2)

        self.fc1 = nn.Linear(latent_dim, 128)
        self.fc2 = nn.Linear(128, 64)
        self.out = nn.Linear(64, self.n_output)

    def forward(self, drug, prot, max_d_node=48, multi_hop_max_dist=20, spatial_pos_max=20):
        x, edge_index, batch, drug_node, drug_attn_bias, drug_spatial_pos, drug_in_degree, drug_out_degree, drug_edge_input = drug.x, drug.edge_index, drug.batch, drug.node, drug.attn_bias, drug.spatial_pos, drug.in_degree, drug.out_degree, drug.edge_input
        x2, edge_index2, batch2, prot_lens, edge_attr2 = prot.x, prot.edge_index, prot.batch, prot.prot_len, prot.edge_attr
        
        drug_attn_bias[:, :][drug_spatial_pos >= spatial_pos_max] = float('-inf')
           


        drug_n_graph, drug_n_node  = drug_node.squeeze(0).size()[:2] 
        ...
        drug_spatial_pos_bias = self.d_spatial_pos_encoder(drug_spatial_pos).permute(0, 3, 1, 2) 
        drug_graph_attn_bias = drug_graph_attn_bias + drug_spatial_pos_bias  
        ...
        drug_spatial_pos[drug_spatial_pos == 0] = 1  
        ...
        drug_edge_input = torch.clamp(drug_edge_input, min=0, max=384)
        ...
        drug_node = torch.clamp(drug_node, min=0, max=1153)
        drug_node_feature = self.d_node_encoder(drug_node.long()).sum(dim=-2)
        ...
        # transfomrer encoder
        drug_output = self.input_dropout(drug_graph_node_feature) 
        drug_output = self.d_encoders(drug_output, drug_graph_attn_bias)  
        ...
        if drug_output.size(1) > x.size(1):
            pad_size =  drug_output.size(1) - x.size(1)
            padding = torch.zeros((x.size(0), pad_size, x.size(2)), device=drug_output.device)
            x = torch.cat([x, padding], dim=1)
            drug_after = torch.cat([drug_after, padding], dim=1)
