...
def drug_embedding(smile, max_node=48):
    ...
    attn_edge_type = torch.zeros([N, N, 64], dtype=torch.float) #64 : embedding size
    embedded_edge_attr = mol_to_single_emb(edge_attr)

    attn_edge_type[edge_index[0, :], edge_index[1, :]] = embedded_edge_attr + 1

    shortest_path_result, path = all_pairs_shortest_paths_dijkstra(adj.numpy())
